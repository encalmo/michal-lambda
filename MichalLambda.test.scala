...
test("MichalLambda should respond with a number of steps") {

    val input = ApiGatewayRequest(
      httpMethod = "POST",
      path = "/",
      body = task
    ).writeAsString

    val output = new MichalLambda().test(input)

    assertEquals(output, "{\"steps\":\"100\"}")
  }
